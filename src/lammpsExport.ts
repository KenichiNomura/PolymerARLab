...
export function buildUFFInput(elements: AtomSymbol[], name: string, counts: LammpsCounts): string {
  ...
  return [
    ...
    "# Minimization and NVT are written to separate XYZ trajectories (.min.xyz / .nvt.xyz).",
    "",
    "units           real",
    "atom_style      full",
    ...
    "",
    "# Energy-minimization trajectory (overlap-relief warm-up + FIRE) -> .min.xyz",
    `dump            mindump all xyz 20 ${name}.min.xyz`,
    `dump_modify     mindump element ${species} sort id first yes`,
    "",
    ...
    "undump          mindump",
    "",
    "# Separate trajectory for the NVT run -> .nvt.xyz",
    `dump            nvtdump all xyz 20 ${name}.nvt.xyz`,
    `dump_modify     nvtdump element ${species} sort id first yes`,
    "",

...
export function buildUFFInput(elements: AtomSymbol[], name: string, counts: LammpsCounts): string {
  ...
  return [
    ...
    "# Warm-up (relieve steric clashes), FIRE minimization, then NVT @ 300 K for 10 ps.",
    "# The full run (warm-up + FIRE minimization + NVT) is written to one extended-XYZ trajectory (.xyz).",
    "",
    "units           real",
    "atom_style      full",
    ...
    "",
    "# Single extended-XYZ trajectory spanning the whole run (warm-up + FIRE + NVT).",
    "# extxyz needs LAMMPS built with the EXTRA-DUMP package (standard in most",
    "# prebuilt LAMMPS distributions); writes a libAtoms/ASE-compatible",
    "# Properties=species:S:1:pos:R:3 header other trajectory viewers can read.",
    `dump            traj all extxyz 20 ${name}.xyz`,
    `dump_modify     traj element ${species} sort id first yes forces no vel no`,
    "",
    "# Relieve any overlapping atoms from the initial chain packing with",
    "# capped-displacement dynamics, so the minimizer cannot blow up on close",
    "# contacts (built polymers often start with steric clashes between units).",
    "fix             warmup all nve/limit 0.1",
    "run             2000",
    "unfix           warmup",
    "",
    ...
    "# Remove overall translational and rotational drift every 100 steps",

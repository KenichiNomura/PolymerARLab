...
export function buildUFFInput(elements: AtomSymbol[], name: string, counts: LammpsCounts): string {
  ...
  return [
    ...
    "# The full run (warm-up + FIRE minimization + NVT) is written to one XYZ trajectory (.xyz).",
    "",
    "units           real",
    "atom_style      full",
    ...
    "",
    "# Single trajectory spanning the whole run (warm-up + FIRE + NVT) -> .xyz",
    `dump            traj all xyz 20 ${name}.xyz`,
